...
edge_train_prop_list = [0.7, 0.3]
edge_valid_prop = 0.1
seed = 76

weight = [1, args.neg_sampling_ratio]
weight = torch.tensor(weight)

for i, edge_train_prop in enumerate(edge_train_prop_list):
    fix_seed(seed)
    print(f'edge_train_prop: {edge_train_prop:.2f}, '
          f'seed: {seed:02d}')
    ...
    # Load method
    model = WSGNN_LP(args, d, n, c, device).to(device)
    criterion = ELBOLPLoss()
    ...
    for epoch in range(args.epochs):
        ...
        loss = criterion(dataset, p_a, q_a, edge_index_train, edge_label_train.long(), weight.to(device))
        loss.backward()
        optimizer.step()

        result = evaluate_lp(model, dataset, edge_index,
                             num_edge_train, num_edge_valid, num_edge_test,
                             edge_label_train, edge_label_valid, edge_label_test)

        if result[1] >= best_edge_val:
            best_edge_val = result[1]
            best_edge_test = result[2]
            best_edge_epoch = epoch

        if (epoch + 1) % args.display_step == 0:
            print(f'Epoch: {epoch:02d}, '
                  f'Loss: {loss:.4f}, '
                  f'LP_Train_Roc: {result[0]:.4f}, '
                  f'LP_Valid_Roc: {result[1]:.4f}, '
                  f'LP_Test_Roc: {result[2]:.4f}')
    print('\n')
    print(f'edge_val: {best_edge_val:.4f}, '
